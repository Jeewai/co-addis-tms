from django.contrib.auth.mixins import AccessMixin
from django.shortcuts import redirect

class OrganiserAndLoginRequiredMixin(AccessMixin):
    """ Verify that the current user is authenticated and is an organiser """
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated or not request.user.is_organiser:
            return redirect("tickets:ticket-list")
        return super().dispatch(request, *args, **kwargs)

class OfficialAndLoginRequiredMixin(AccessMixin):
    """ Verify that the current user is authenticated and is an official """
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated or not request.user.is_official:
            return redirect("tickets:ticket-list")
        return super().dispatch(request, *args, **kwargs)

